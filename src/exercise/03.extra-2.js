...
function PokemonCacheProvider({children}) {
  ...
  return <PokemonCacheContext.Provider children={children} value={value}/>
}

function useCachedPokemon() {
  const ctx = useContext(PokemonCacheContext);
  if (!ctx) {
    throw new Error(
      'useCachedPokemon may only be used from within a (child of a) <PokemonCacheProvider>',
    );
  }
  return ctx;
}
